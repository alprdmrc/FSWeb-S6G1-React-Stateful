...
export default function Programcilar() {
  ...
  const [kutlanan, setKutlanan] = useState(null)
  ...
  const oneCikaninIsmi = () => {
    ...
    return programmers[kutlanan-1].isim
  };

  const stil = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: !kutlanan ? 'royalblue' : 'gold', // 🤔 kutlarken renk gold'a dönecek
  };
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.isim} <button onClick={() => {setKutlanan(dev.id) }}>Kutla</button>
            </div>
          )
        }
      </div>
